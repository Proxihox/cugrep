 ...
 */
bool match(const char* data, long long startPos) {
    long long matchLen = 0;
...
/**
 * Main CUDA kernel for parallel pattern searching
 * Each thread processes a chunk of the input text
 */
__global__ void cudaSearchFileNew(const char* data, long long block_size, 
...
void fileMap(const string& filePath) {
    ...
    while(total_threads > 0){
        ...
        cudaSearchFileNew<<<n_blocks,threads_per_block>>>(d_data,block_size,memsize,memiter);
